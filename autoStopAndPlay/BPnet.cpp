...
double net_test(Net net, int n_in, int n_out, int n_hidden) {
    ...
    for (t = 0; t < all; t++) {
        ...
        //printf("----output %lf and %lf\n", finalOutput[0], finalOutput[1]);
        // printf("output %d vs %lf with %lf and %lf\n", pos, tests[t][n_in], finalOutput[0], finalOutput[1]);
        if (pos == tests[t][n_in]) {
...
Net BACKPROPAGATION(dVector examples, double eta, int n_in, int n_out, int n_hidden) {
    ...
    while(END_FLAG ==0 && step < MAX_STEP) {
        ...
        for (e = 0; e < t; e++) {
            ...
            // printf("       output %lf and %lf\n", deltaOfOutput[0], deltaOfOutput[1]);
        ...
        if (correctRate > 0.98) {
            ...
            for (int h = 0; h < hidden.size(); h++) {
                std::vector<double> weight = hidden[h].weight;
                for (int i = 0; i < weight.size(); i++)
                    printf("%lf ", weight[i]);
                printf("\n");
            }
            for (int o = 0; o < out.size(); o++) {
                std::vector<double> weight = out[o].weight;
                for (int i = 0; i < weight.size(); i++)
                    printf("%lf ", weight[i]);
                printf("\n");
            }
...
int main(int argc, char* argv[]) {
    ...
    Net ans = BACKPROPAGATION(examples, 0.02, 768, 2, 26);
    printf("\nTrain over, the accuracy is %lf\n", net_test(ans, 768, 2, 26));
    return 0;
}
